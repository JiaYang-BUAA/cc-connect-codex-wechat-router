...
> 当前稳定版本：通知器 `1.3.0`，配套 cc-connect 路由补丁 `v1.4.1+qr15`。
...
- `/rw` 显示 5h/7d 剩余额度和刷新时间，以及当前置顶顺序、运行时长和排队数。
...
3. 在微信中发送 `/rw`。通知器会先显示 Codex 的 5h/7d 剩余额度和刷新时间，再按 Codex Desktop 当前侧边栏的置顶顺序列出任务编号、运行状态和运行时长。
4. 记住任务编号后，可以用 `/rw编号 内容` 直接向该任务发送新消息。

### 查看状态

发送 `/rw` 可以查看 5h/7d 剩余额度、对应刷新时间以及所有置顶任务，包括单独置顶的定时任务。运行中的任务显示当前处理时间，未运行的任务显示“空闲”；没有置顶任务时仍会显示额度和推送开关，并明确提示当前没有置顶任务。编号按当前置顶顺序计算，任务取消置顶或归档后，编号可能变化。额度暂时读取失败时会显示“Codex额度：暂时无法读取”，不影响下面的任务状态。
...
### 微信 `context_token` 过期与积压消息汇总

`context_token` 是微信网关随用户消息下发的临时回复凭证。cc-connect 必须携带它，才能把 Codex 的最终答复主动发送到对应微信会话。它不是微信登录凭证、Codex API Key 或 Codex 额度令牌，也不会影响 Codex 任务本身是否继续运行。

当你给机器人发送微信消息时，微信网关可能下发新的 `context_token`，cc-connect 会将其缓存起来供后续推送使用。把令牌保存到本地只能让 cc-connect 重启后继续读取它，不能延长微信服务器端的有效期。有效期和失效条件由微信网关控制，目前没有公开、固定的过期时长；长时间没有向机器人发送消息、微信会话或登录状态变化、网关轮换和风控都可能使旧令牌不再被接受。因此不能依靠“每隔固定几小时刷新一次”来彻底避免过期。

令牌失效时通常会出现以下现象：Codex 对话已经完成，但微信没有收到最终答复；通知器或 cc-connect 日志出现 `ret=-2`、`expired context_token` 或“需要用户重新发送消息”的错误。此时不是 Codex 任务失败，也不是额度耗尽，而是微信暂时拒绝机器人使用旧凭证主动发消息。
